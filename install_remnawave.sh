...
remove_script() {
    # Do not orphan policy routing, systemd units or Remnawave profiles by
    # deleting this custom build before its managed integrations are removed.
    if [ -e "${DIR_REMNAWAVE}amneziawg_remnawave/state.json" ] \
        || [ -e "${DIR_REMNAWAVE}cascade_vless/state.json" ]; then
        echo -e "${COLOR_RED}${LANG[CUSTOM_INTEGRATIONS_REMOVE_FIRST]}${COLOR_RESET}"
        return 1
    fi
...
install_script_if_missing() {
    ...
    # When launched from this modified archive, install the bundled script,
    # languages and modules instead of downloading the upstream build.
    if [ -d "${SCRIPT_DIR}/src" ] && [ -f "${SCRIPT_DIR}/src/modules/cascade_vless.sh" ]; then
        if [ "${SCRIPT_DIR}/install_remnawave.sh" != "${DIR_REMNAWAVE}remnawave_reverse" ]; then
            cp "${SCRIPT_DIR}/install_remnawave.sh" "${DIR_REMNAWAVE}remnawave_reverse"
        fi
        cp -a "${SCRIPT_DIR}/src/." "${DIR_REMNAWAVE}/"
        chmod +x "${DIR_REMNAWAVE}remnawave_reverse"
        find "${DIR_REMNAWAVE}" -type f -name '*.sh' -exec chmod 755 {} + 2>/dev/null || true
        ln -sf "${DIR_REMNAWAVE}remnawave_reverse" /usr/local/bin/remnawave_reverse
    elif [ ! -f "${DIR_REMNAWAVE}remnawave_reverse" ] || [ ! -e "/usr/local/bin/remnawave_reverse" ]; then
...
show_menu() {
    ...
    echo -e "${COLOR_YELLOW}12. ${LANG[MENU_12]}${COLOR_RESET}" # VLESS server-side cascade
    echo -e "${COLOR_YELLOW}13. ${LANG[MENU_13]}${COLOR_RESET}" # AmneziaWG 3.0 inbound to Remnawave
    echo -e ""
    echo -e "${COLOR_YELLOW}0. ${LANG[EXIT]}${COLOR_RESET}"
...
pull_compose_images() {
    local tmpfile pid rc
    tmpfile="$(mktemp)"
    docker compose pull "$@" >"$tmpfile" 2>&1 &
    pid=$!
    spinner "$pid" "${LANG[PULLING_CONTAINER_IMAGES]}"
    wait "$pid"
    rc=$?
    if [ "$rc" -ne 0 ]; then
        cat "$tmpfile"
        rm -f "$tmpfile"
        echo -e "${COLOR_RED}${LANG[PULL_CONTAINER_IMAGES_FAILED]}${COLOR_RESET}"
        return "$rc"
    fi
    rm -f "$tmpfile"
    return 0
...
install_docker_official_apt() {
    ...
    if [ ! -r /etc/os-release ]; then
        echo -e "${COLOR_RED}${LANG[DOCKER_UNSUPPORTED_OS]}${COLOR_RESET}" >&2
        return 1
    fi

    . /etc/os-release
    if [ "${ID:-}" != "ubuntu" ]; then
        echo -e "${COLOR_RED}${LANG[DOCKER_UNSUPPORTED_OS]}: ${PRETTY_NAME:-unknown}${COLOR_RESET}" >&2
        return 1
    fi

    case "${VERSION_ID:-}" in
        22.04|24.04|25.04|26.04) ;;
        *) echo -e "${COLOR_YELLOW}${LANG[DOCKER_OS_NOT_TESTED]}: ${VERSION_ID:-unknown}${COLOR_RESET}" ;;
    esac
    ...
    if [ -z "$codename" ]; then
        echo -e "${COLOR_RED}${LANG[DOCKER_CODENAME_ERROR]}${COLOR_RESET}" >&2
        return 1
    fi
    ...
    local key_tmp
    key_tmp="$(mktemp)"
    if ! curl --proto '=https' --tlsv1.2 -fsSL https://download.docker.com/linux/ubuntu/gpg -o "$key_tmp"; then
        rm -f "$key_tmp"
        echo -e "${COLOR_RED}${LANG[ERROR_DOWNLOAD_DOCKER_KEY]}${COLOR_RESET}" >&2
        return 1
    fi
    gpg --dearmor --yes -o /etc/apt/keyrings/docker.gpg "$key_tmp" || { rm -f "$key_tmp"; return 1; }
    rm -f "$key_tmp"
...
EOF
    ...
    docker_engine_healthy || return 1
    docker run --rm hello-world >/dev/null 2>&1 || {
        echo -e "${COLOR_RED}${LANG[DOCKER_TEST_FAILED]}${COLOR_RESET}" >&2
        return 1
    }
...
ensure_docker_ready() {
    if docker_engine_healthy; then
        echo -e "${COLOR_GREEN}${LANG[DOCKER_FOUND]}: $(docker --version 2>/dev/null); $(docker compose version 2>/dev/null)${COLOR_RESET}"
        return 0
    fi

    # Docker CLI may exist while daemon or Compose is missing.
    if command -v docker >/dev/null 2>&1; then
        ...
        echo -e "${COLOR_YELLOW}${LANG[DOCKER_BROKEN_REINSTALL]}${COLOR_RESET}"
    else
        echo -e "${COLOR_YELLOW}${LANG[DOCKER_NOT_FOUND]}${COLOR_RESET}"
    fi
...
install_packages() {
    ...
    # Docker preflight. Preserve a healthy existing Docker installation; otherwise
...
check_certificates() {
    ...
    if [ -n "$live_dir" ] && [ -d "$live_dir" ]; then
        ...
        for file in "${files[@]}"; do
            ...
            if [ ! -L "$file_path" ]; then
                fix_letsencrypt_structure "$(basename "$live_dir")" "$target_dir"
                if [ $? -ne 0 ]; then
...
fix_letsencrypt_structure() {
    ...
    local expected_hook="renew_hook = sh -c 'cd $target_dir && docker compose up -d --no-deps --force-recreate remnawave-nginx'"
...
append_nginx_cert_mounts() {
    ...
    grep -Fqx -- "$fullchain_mount" "$compose_file" 2>/dev/null || printf '%s\n' "$fullchain_mount" >> "$compose_file"
    grep -Fqx -- "$privkey_mount" "$compose_file" 2>/dev/null || printf '%s\n' "$privkey_mount" >> "$compose_file"
}
...
handle_certificates() {
    ...
    if [ ! -f "$compose_file" ]; then
        echo -e "${COLOR_RED}docker-compose.yml not found: $compose_file${COLOR_RESET}"
        return 1
    fi
    ...
    for domain in "${!domains_to_check_ref[@]}"; do
        if ! check_certificates "$domain" "$target_dir"; then
    ...
    if [ "$need_certificates" = true ] && [ "$cert_method" == "1" ]; then
        ...
        for domain in "${!unique_domains[@]}"; do
            ...
            if [ -z "${cert_domains_added[$domain]}" ]; then
                append_nginx_cert_mounts "$compose_file" "$domain"
        ...
        for domain in "${!domains_to_check_ref[@]}"; do
            ...
            if [ -z "${cert_domains_added[$cert_domain]}" ]; then
                append_nginx_cert_mounts "$compose_file" "$cert_domain"
    ...
    local cron_command
    if [ "$cert_method" == "2" ]; then
        cron_command="ufw allow 80 && /usr/bin/certbot renew --quiet && ufw delete allow 80 && ufw reload && cd $target_dir && docker compose up -d --no-deps --force-recreate remnawave-nginx"
    else
    ...
    for domain in "${!unique_domains[@]}"; do
        if [ -f "/etc/letsencrypt/renewal/$domain.conf" ]; then
            desired_hook="renew_hook = sh -c 'cd $target_dir && docker compose up -d --no-deps --force-recreate remnawave-nginx'"
...
node_local_runtime_ready() {
    local frontend_container="$1"

    [ "$(docker inspect -f '{{.State.Running}}' remnanode 2>/dev/null)" = "true" ] || return 1
    [ "$(docker inspect -f '{{.State.Running}}' "$frontend_container" 2>/dev/null)" = "true" ] || return 1
    ss -H -lnt 2>/dev/null | awk '$4 ~ /:2222$/ {found=1} END {exit !found}' || return 1
    [ -S /dev/shm/nginx.sock ] || return 1
    return 0
}

wait_for_node_local_runtime() {
    local frontend_container="$1"
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
        ...
        if node_local_runtime_ready "$frontend_container"; then
    ...
    echo -e "${COLOR_YELLOW}remnanode:${COLOR_RESET} $(docker inspect -f '{{.State.Status}}' remnanode 2>/dev/null || echo missing)"
    echo -e "${COLOR_YELLOW}${frontend_container}:${COLOR_RESET} $(docker inspect -f '{{.State.Status}}' "$frontend_container" 2>/dev/null || echo missing)"
    ss -lntp 2>/dev/null | grep ':2222' || true
    docker logs "$frontend_container" --tail 20 2>&1 || true
    docker logs remnanode --tail 20 2>&1 || true
    return 1
}

repair_existing_node_nginx_cert_mounts() {
...
load_install_panel_node_module() { load_module "install_panel" "nginx" "${1:-false}"; }
load_install_panel_module() { load_module "install_panel_node" "nginx" "${1:-false}"; }
...
load_caddy_module() { load_module "install_panel" "caddy" "${1:-false}"; }
load_caddy_panel_module() { load_module "install_panel_node" "caddy" "${1:-false}"; }
...
load_cascade_vless_module() { load_module "cascade_vless" "modules" "${1:-false}"; }
load_amneziawg_inbound_module() { load_module "amneziawg_inbound" "modules" "${1:-false}"; }
...
check_os
install_script_if_missing
repair_existing_node_nginx_cert_mounts || true
check_update_status
show_menu
